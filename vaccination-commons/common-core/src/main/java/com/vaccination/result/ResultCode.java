...
public enum ResultCode implements IResultCode, Serializable {
    ...
    USER_EXIST("A0204","用户已经存在"),
    FAIL_REGISTER("A0205","失败，请重试"),

    USERNAME_OR_PASSWORD_ERROR("A0210", "用户名或密码错误"),
    INPUT_PASSWORD_EXCEED_LIMIT("A0211", "用户输入密码次数超限"),
    CLIENT_AUTHENTICATION_FAILED("A0212", "客户端认证失败"), // *
    TOKEN_INVALID_OR_EXPIRED("A0230", "token无效或已过期"),
    PASSWORD_UN_MODEFID("A0231", "请修改默认密码!"),

    SMSCODE_REPEAT("A0235", "验证码已经发送"),
    SMSCODE_ERROR("A0235", "验证码错误"),
    ...
    SYSTEM_EXECUTION_ERROR("B0001", "执行错误"),
    SYSTEM_EXECUTION_TIMEOUT("B0100", "执行超时"),
    ...
    public static String getMessage(String val) {
        for (ResultCode a : values()) {
            if (a.getCode().equals(val)) {
                return a.getMsg();
            }
        }
        return null;
